...
export function LedgerView() {
  ...
  const [budgetAsDefault, setBudgetAsDefault] = useState(false);
  ...
  // 同步拉回对端新账目后重载本页（账本数据为页内本地状态，不进全局 store）。
  useEffect(() => {
  ...
  // 移动端：系统返回键与抽屉内右滑手势都走「关闭抽屉」，不退出应用。
  const drawerRef = useRef<HTMLElement>(null);
  useEffect(() => {
    if (!drawer || !isMobileShell()) return;
    const unregister = pushBackHandler(() => {
      setDrawer(false);
      return true;
    });
    const detach = drawerRef.current
      ? attachSwipeRightToClose(drawerRef.current, () => setDrawer(false))
      : undefined;
    return () => {
      unregister();
      detach?.();
  ...
  // SelectMenu/DatePicker/TimePicker 的弹层经 portal 挂在 body 上,不属于抽屉内部,
  // 要一并豁免,否则点选账户选项或日历选日期会被当成"点了外面"而整块收起、选择丢失。
  useEffect(() => {
    if (!drawer) return;
    const onPointerDown = (event: PointerEvent) => {
      if (!(event.target instanceof Element)) return;
      if (event.target.closest(".ledger-drawer")) return;
      if (event.target.closest(".modal-backdrop, .row-menu, .row-menu-backdrop, .select-menu, .date-picker-pop, .time-picker-pop")) return;
      setDrawer(false);
    };
  ...
  function startBudgetEdit() {
    setBudgetDraft(budget ? String(budget / 100) : "");
    setBudgetAsDefault(false);
    setBudgetError("");
    setBudgetEditing(true);
  }

  async function saveBudget() {
    const cents = budgetDraft.trim() === "0" ? 0 : parseAmountToCents(budgetDraft);
    if (cents === null) { setBudgetError("请输入有效金额，设为 0 可关闭当月预算"); return; }
    try {
      await setLedgerBudget(month, cents, budgetAsDefault);
      setBudgetEditing(false);
      setNotice({ text: cents === 0 ? "已关闭当月预算" : "预算已保存" });
      await load();
    } catch (reason) {
      setBudgetError(reason instanceof Error ? reason.message : "保存失败");
    }
  }
  ...
  return (
    <main className="main-workspace ledger-view">
      {/* 与「今日」同构的标题区:h2 标题 + 副标题,右侧为月份切换与记账入口。 */}
      <div className="workspace-top">
        <div>
          <h2>收支总览</h2>
          <p className="workspace-subtitle">观水有术，必观其澜。看见钱的去向，不评判每一笔生活。</p>
        </div>
      ...
      <section className="today-hero guide-hero">
        <div className="today-hero-copy">
          <span className="today-eyebrow">观澜 · 收支</span>
          <h3>看清流向，再谈安排。</h3>
          <p className="today-hero-note">月度摘要、分类占比与趋势都在一页；截图分享前可用「隐藏金额」。</p>
        </div>
      </section>
      ...
      <>
        <section className="ledger-stat-grid" aria-label="本月收支摘要">
          ...
          <article>
            <span className="ledger-stat-head">预算进度
              <button type="button" className="ledger-budget-set" onClick={() => (budgetEditing ? setBudgetEditing(false) : startBudgetEdit())}>{budgetEditing ? "收起" : "设置"}</button>
            </span>
            {/* 展示内容常驻,预算编辑器以浮层覆盖其上:展开/收起都不改变统计卡高度。 */}
            <>
              <strong>{budget ? `${Math.round(budgetRatio * 100)}%` : "未设置"}</strong>
              <div className="ledger-progress"><i style={{ width: `${Math.min(100, budgetRatio * 100)}%` }} /></div>
              <small>{budget === 0 ? "点「设置」定一个月度边界" : budgetRatio > 1 ? `已超支 ${formatLedgerMoney(expense - budget, hidden)}` : budgetRatio === 1 ? "本月预算已用完" : `还可用 ${formatLedgerMoney(budget - expense, hidden)}`}</small>
            </>
            {budgetEditing ? (
              <div className="ledger-budget-editor">
                <div className="ledger-budget-input compact"><span>¥</span><input inputMode="decimal" value={budgetDraft} placeholder="预算金额" onChange={(event) => { setBudgetDraft(event.target.value.replace(/[^\d.]/g, "")); setBudgetError(""); }} onKeyDown={(event) => { if (event.key === "Enter") void saveBudget(); }} /></div>
                <label className="ledger-checkbox"><input type="checkbox" checked={budgetAsDefault} onChange={(event) => setBudgetAsDefault(event.target.checked)} />设为以后月份的默认预算</label>
                {budgetError && <p className="ledger-error" role="alert">{budgetError}</p>}
                <div className="ledger-budget-editor-actions">
                  <button className="btn-ghost" onClick={() => setBudgetEditing(false)}>取消</button>
                  <button className="btn-primary" onClick={() => void saveBudget()}>保存预算</button>
                </div>
                <small>当月预算优先于默认预算；填 0 可关闭当月预算。</small>
              </div>
            ) : null}
          </article>
        </section>
      ...
      {drawer && <aside ref={drawerRef} className="ledger-drawer" role="dialog" aria-labelledby="ledger-drawer-title"><header className="ledger-drawer-head"><div><p className="ledger-eyebrow">{editing ? "编辑记录" : "快速记账"}</p><h3 id="ledger-drawer-title">{editing ? "修改这一笔" : "记一笔"}</h3></div><button type="button" className="ledger-close" aria-label="关闭记账面板" onClick={() => setDrawer(false)}>✕</button></header><div className="ledger-drawer-body"><div className="ledger-kind-tabs"><button className={draft.kind === "expense" ? "active" : ""} onClick={() => setKind("expense")}>支出</button><button className={draft.kind === "income" ? "active" : ""} onClick={() => setKind("income")}>收入</button></div><label className="ledger-amount"><span>金额</span><div><b>¥</b><input ref={amountRef} value={draft.amount} inputMode="decimal" autoComplete="off" placeholder="0.00" aria-describedby={error ? "ledger-error" : undefined} onChange={(event) => { const value = event.target.value.replace(/[^\d.]/g, ""); if (value.length <= 11) setDraft((current) => ({ ...current, amount: value })); }} onKeyDown={(event) => { if (event.key === "Enter") void save(); }} /></div></label><div className="ledger-quick-amounts">{[10, 20, 50, 100].map((value) => <button key={value} onClick={() => setDraft((current) => ({ ...current, amount: String(value) }))}>¥{value}</button>)}</div><fieldset><legend>分类</legend><div className="ledger-choice-grid">{activeCategories.map((item) => <button type="button" key={item.id} className={draft.categoryId === item.id ? "active" : ""} onClick={() => setDraft((value) => ({ ...value, categoryId: item.id }))}><i style={{ color: item.color }}>{ICONS[item.icon] ?? "·"}</i>{item.name}</button>)}</div></fieldset><div className="ledger-form-row"><label>日期<DatePicker value={draft.date} onChange={(date) => setDraft((value) => ({ ...value, date }))} ariaLabel="记账日期" /></label><label>账户<SelectMenu ariaLabel="账户" value={String(draft.accountId)} onChange={(accountId) => setDraft((current) => ({ ...current, accountId: Number(accountId) }))} options={activeAccounts.map((item) => ({ value: String(item.id), label: `${accountGlyph(item.kind)} · ${item.name}` }))} /></label></div><label>备注（可选）<input maxLength={60} value={draft.note} onChange={(event) => setDraft((value) => ({ ...value, note: event.target.value }))} /></label>{error && <p className="ledger-error" id="ledger-error" role="alert">{error}</p>}</div><footer className="ledger-drawer-footer"><button className="btn-ghost" onClick={() => setDrawer(false)}>取消</button><button className="btn-primary" onClick={() => void save()}>保存记录</button></footer></aside>}
