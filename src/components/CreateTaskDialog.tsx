    ...
    <div className="modal-backdrop" onMouseDown={close}>
      ...
      >
        ...
        <div className="create-task-grid">
          ...
          <label>标签<SelectMenu ariaLabel="标签" value={selectedTagIds[0] ?? ""} onChange={(value) => setSelectedTagIds(value ? [value] : [])} options={[{ value: "", label: "无标签" }, ...tags.map((tag) => ({ value: tag.id, label: tag.name }))]} /></label>
          {dueDate ? (
            <label>
              重复
              <SelectMenu
