...
export function AnniversariesView() {
  ...
  return (
    <main className="main-workspace moments-page anniversaries-page">
      {/* 与「今日」同构的标题区:h2 标题 + 副标题,右侧直达「新增纪念日」。 */}
      <div className="workspace-top">
        <div>
          <h2>纪念日</h2>
          <p className="workspace-subtitle">重要的日子都在这里，离现在最近的会先被看见。</p>
        </div>
      ...
      <section className="today-hero guide-hero">
        <div className="today-hero-copy">
          <span className="today-eyebrow">有恒 · 纪念日</span>
          <h3>把值得记住的日子，好好收藏。</h3>
          <p className="today-hero-note">公历、农历都能记；日子按远近排序，未来三十天内的会单独列出一栏。</p>
        </div>
      </section>
          ...
          {calendar === "solar" ? (
            ...
            <div className="anni-lunar-pickers">
              ...
              <SelectMenu
                ...
                onChange={(value) => setLunarMonth(Number(value))}
                options={LUNAR_MONTH_LABELS.map((label, index) => ({ value: String(index + 1), label }))}
              />
              <SelectMenu
                className="field"
                ariaLabel="农历日"
                value={String(lunarDay)}
                onChange={(value) => setLunarDay(Number(value))}
                options={LUNAR_DAY_LABELS.map((label, index) => ({ value: String(index + 1), label }))}
              />
