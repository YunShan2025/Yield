...
import { WEEKLY_CATEGORIES } from "@/lib/weeklyChecklist";
import { getDb } from "./client";
import { addGoalEntry, removeGoalEntryBySource } from "./growth";

/* Tags */

// 默认标签与周清单四分类同名同色，顺序即产品约定的默认展示顺序。
const DEFAULT_TAGS: { name: string; color: string }[] = ["工作", "生活", "健康", "学习"].map(
  (name) => ({
    name,
    color:
      WEEKLY_CATEGORIES.find((category) => category.label === name)?.color ??
      "#5B8FF9",
  }),
);
...
export async function fetchTags(): Promise<Tag[]> {
  const db = await getDb();
  return db.select<Tag[]>("SELECT * FROM tags ORDER BY created_at ASC, name ASC");
}
