"""Optional HiDPI NSIS installer bitmaps for 有秋 (Yield).
...
def make_header() -> None:
    ...
    draw.text((text_x, 42), "有秋", fill=TEXT, font=load_font(42, bold=True))
    draw.text((text_x, 100), "Yield", fill=TEXT_MUTED, font=load_font(27))
...
def make_sidebar() -> None:
    ...
    # Titles
    draw.text((w // 2, 400), "有秋", fill=TEXT, font=load_font(38, bold=True), anchor="mm")
    draw.text((w // 2, 454), "Yield", fill=TEXT_MUTED, font=load_font(26), anchor="mm")

    # Pill
    pill = (90, 520, 402, 590)
    draw.rounded_rectangle(pill, radius=35, fill=WHITE)
    draw.rounded_rectangle(pill, radius=35, outline=(255, 255, 255), width=3)
    draw.text((w // 2, 555), "服田力穑 · 乃亦有秋", fill=ACCENT_DARK, font=load_font(28, bold=True), anchor="mm")
